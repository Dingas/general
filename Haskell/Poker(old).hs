...
deal xs = do
  let evens = map fst $ filter (even . snd) $ zip xs [1..]
  let odds = map fst $ filter (odd . snd) $ zip xs [1..]
  
  
  let unsortedHand1 = map getCard evens
  let unsortedHand2 = map getCard odds
  let sortedHand1 = cardSort unsortedHand1
  let sortedHand2 = cardSort unsortedHand2
  
  bestHand sortedHand1 sortedHand2
{-
((2,"D"),15)
fst $ -> (2,"D")
fst $ fst $ head x -> 2
